...
class UnitreeA1EnvCfg(DirectRLEnvCfg):
    ...
    feet_air_time_reward_scale = 0.5 # 0.5
    undersired_contact_reward_scale = -20.0 # -1.0
...
class UnitreeA1Env(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        is_contact = (
            torch.max(torch.norm(net_contact_forces[:, :, self._undesired_contact_body_ids], dim=-1), dim=1)[0] > 1e-5
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        height = self._robot.data.root_state_w[:,2]
        died = torch.logical_or(
            torch.any(torch.max(torch.norm(net_contact_forces[:, :, self._base_id], dim=-1), dim=1)[0] > 1e-3, dim=1), 
            height<0.1)
