...
class DifferentialIKController:
    ...
    def compute(
        ...
        if "position" in self.cfg.command_type:
            ...
            # print(f"Destination : {self.ee_pos_des[0]}")
            # print(f"Now : {ee_pos[0]}")
    ...
    def _compute_delta_joint_pos(self, delta_pose: torch.Tensor, jacobian: torch.Tensor) -> torch.Tensor:
        ...
        if self.cfg.ik_method == "pinv":  # Jacobian pseudo-inverse
            ...
            delta_joint_pos = delta_joint_pos.squeeze(-1)
            # print(f"Delta pose : {delta_pose[0]}")
            # print(f"Jacobian : {jacobian[0]}")
            # print(f"Before Inverse : {(jacobian @ jacobian_T + lambda_matrix)[0]}")
            # print(f"After Inverse : {torch.inverse(jacobian @ jacobian_T + lambda_matrix)[0]}")
        else:
